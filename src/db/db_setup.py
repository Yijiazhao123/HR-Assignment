...
engine = create_engine('sqlite:///utsc-exercise.db')
...
def ingest_csv_data(filename: str):
    ...
    """
    # 1. Connect to the database file "utsc-exercise.db" using the connect_to_db function
    db_connection = connect_to_db("utsc-exercise.db")
    
    # 2. Use pandas to load the CSV 'filename' into a dataframe, and clean it if needed
    df = pd.read_csv(filename)
    cleaned_df = remove_unnamed_columns(df)  # Remove unnamed columns if present
    
    # 3. Insert the data into the SQL database
    insert_employee_data_into_db(db_connection, cleaned_df)  # Correct function name
    
    # 4. Move the file to the 'hist' folder to indicate it has been processed
